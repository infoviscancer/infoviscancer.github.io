...
var data = _to_data(0,1, all_data); //0= normal, 1=tumoral
...
sg.selectAll('.slopes')
	.data(data).enter().append('svg:line')
		...
		.on("mouseover", function (d) {
				  ...
				  d3.select("#tooltip")
					.style("visibility", "visible")
					.html(d.label +'<br/>Avg Normal: ' + d.left + '<br/>Avg Tumoral: ' + d.right)
					.style("top", function () { return (d3.event.pageY - 100)+"px"})
					.style("left", function () { return (d3.event.pageX - 100)+"px";})
				})
		.on("mouseout", function (d) { 
...
function draw_line(left, right) {
return sg.append('svg:line')
		.attr('x1', LEFT_MARGIN)
		.attr('x2', WIDTH-RIGHT_MARGIN)
		.attr('y1', function(d,i){
			return y(left)})
